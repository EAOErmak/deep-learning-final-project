...
class BuyFeatureExtractor:
    ...
    def extract_from_tick_rows(self, tick_rows: pd.DataFrame, perspective_steamid: int) -> np.ndarray:
        ...
        team_weapon_ids = [weapon_to_id(player.weapon) / 17.0 for player in team_players]
        ...
        values = [self_player.money / MONEY_SCALE, self_player.armor / 100.0, 1.0 if self_player.has_helmet else 0.0, weapon_to_id(self_player.weapon) / 17.0, *team_money[:5], *team_weapon_ids[:5], *team_armor[:5], *team_helmets[:5], float(game_state.round.round_number) / 30.0, float(losing_streak) / 10.0, 1.0 if self_player.in_buy_zone else 0.0]
